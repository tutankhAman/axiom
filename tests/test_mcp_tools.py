...
def test_set_zone_setpoint_clamping() -> None:
    ...
    # Set simulated state at hour 10.0 (off-peak occupied, 25.5-26.5°C band)
    bridge.update_state(
        SimulationState(
            sim_time_hours=10.0,
            outdoor_temp=28.0,
...
def test_set_zone_setpoint_peak_shedding() -> None:
    bridge = StateBridge()
    context = MCPContext(bridge)
    # Set simulated state at hour 15.0 (peak occupied, 27.0-28.5°C band)
    bridge.update_state(
        SimulationState(
            sim_time_hours=15.0,
            outdoor_temp=33.0,
    ...
    set_zone_setpoint(
        ...
        heating_c=15.0,
        cooling_c=28.0,
        reason="Peak shedding operation",
    )

    commands = bridge.get_actuation_commands()
    assert commands["HTGSETP_SCH_NO_OPTIMUM"] == 15.0
    assert commands["CLGSETP_SCH_NO_OPTIMUM"] == 28.0
